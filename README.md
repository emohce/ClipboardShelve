...
## 快捷键（默认）
- 主界面：Tab/Shift+Tab 切分页；Alt+1~9 切标签；Ctrl+F 搜索；Esc 退出；↑/↓/Ctrl+K/J 导航；← 全文预览；→ 操作抽屉；Enter 复制；Ctrl+Enter 复制并锁定；Ctrl+C 复制；Ctrl+S 收藏；Ctrl+U 锁定；Shift+Del/Backspace 开清除框；Del/Backspace 删除；Ctrl+Del/Backspace 强制删锁定；空格多选；Shift 预览图片；Ctrl+1~9 快速复制；Ctrl+Shift+1~9 抽屉子功能 @src/global/hotkeyBindings.js#84-139.
