...
| EM-2026-04-09-alias-prompt-hotkey-layer | `environment-assumption`, `framework-misuse`, `runtime-path-mismatch` | F2 已触发但别名编辑不可见；弹窗输入态与主层 Enter 快捷键冲突 | `src/cpns/ClipItemList.vue`, `src/global/hotkeyRegistry.js` | `window.prompt`, `ElMessageBox.prompt`, `list-enter`, `list-save-by-alias`, `uTools` | 在插件容器内继续使用 `window.prompt` 并忽略热键层级冲突 | 使用 `ElMessageBox.prompt` + 弹窗打开态短路主层 Enter 类快捷键 | 若“按键触发但无可见交互”，先验 UI 容器能力与热键层级隔离，不先继续微调业务逻辑 | [EM-2026-04-09-alias-prompt-hotkey-layer](../vibe-doc/ai-error-memory/2026-04-09-alias-prompt-hotkey-layer.md) |
