...
- `历史规则目录`: 11 Markdown files
- `vibe/knowledge/legacy/`: 21 Markdown files
...
```text
stale=30
broken_or_nonportable=676
duplicate_headings=56
...
- Deep historical Markdown backlog after cleanup: broken links `0`, duplicate headings `0`, sensitive findings `0`, other portability findings `0`.
- Safe automatic relative-link repair was run after the CodeNote audit script learned root-relative fallback.
- Historical Markdown links and repeated headings were normalized without deleting legacy documents.
