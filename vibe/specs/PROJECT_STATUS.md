...
Current focus: 配置文件预览支持 — **已实现并通过 `node test-file-preview.mjs`、`node test-preview-layout.mjs`、`node test-preview-scroll.mjs`、`pnpm run build`；`.conf` / `.cfg` / `.properties` / `.toml` / `.env.*` 等配置类文本文件进入既有富预览分类、限制、缓存和保守内容识别链路；待 uTools 真实文件手工复测** ([../knowledge/technical-details.md](../knowledge/technical-details.md), [260616-rich-file-preview/01-spec.md](260616-rich-file-preview/01-spec.md)).

Current core task: 配置文件预览支持。Authoritative docs: [../knowledge/technical-details.md](../knowledge/technical-details.md), [260616-rich-file-preview/01-spec.md](260616-rich-file-preview/01-spec.md), [260616-rich-file-preview/04-verify.md](260616-rich-file-preview/04-verify.md).
