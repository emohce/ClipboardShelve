...
## 2. 依赖策略
- 直接依赖：`pdfjs-dist`、`markdown-it`、`dompurify`、`@asciidoctor/core`、`read-excel-file`、`papaparse`、`mammoth`、`jszip`。
...
- TXT/MD/ADOC 默认只读取首段 1MB 用于预览；CSV 默认只读取首段 512KB 后解析前 100 行，避免大文本和宽表格整文件阻塞。
